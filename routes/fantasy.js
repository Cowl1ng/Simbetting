...
router.get('/list', ensureAuthenticated, async (req, res) => {
  ...
  try {
    const fantasyGamesCompleted = await FantasyGame.find({
      completed: true,
    }).limit(10)
...
router.post('/new', ensureAuthenticated, async (req, res) => {
  ...
  if (user.name == 'Luke ' || 'Tim') {
    ...
    await FantasyGame.updateMany(
      { game_week: { $lt: gameWeek } },
      { $set: { completed: true, started: gameStarted } }
    )
    await FantasyGame.updateMany(
      { game_week: { $gte: gameWeek } },
      { $set: { completed: false, started: gameStarted } }
    )
...
router.get('/:id', ensureAuthenticated, async (req, res) => {
  try {
    ...
    const users = await User.findById(req.user.id)
    const userFantasyBets = await FantasyBet.find({
      user: users.id,
      game: fantasygame.id,
    })
